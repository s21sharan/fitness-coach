# Research Paper Pipeline Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build an automated pipeline to collect ~80 exercise science papers, embed them into Supabase pgvector, and add a `search_research` tool to the AI coach.

**Architecture:** Two-phase Python CLI (discover → ingest) writes to Supabase. TypeScript coach tool queries pgvector via RPC for semantic search, using OpenAI embeddings for both ingestion and query-time.

**Tech Stack:** Python 3 (requests, docling, openai, supabase, tqdm), Supabase pgvector (halfvec), OpenAI text-embedding-3-small, Vercel AI SDK v6 (@ai-sdk/openai), Next.js App Router
...
**Files:**
- Create: `supabase/migrations/007_research_knowledge_base.sql`
...
-- GIN index for future full-text search
create index research_chunks_fts_idx on research_chunks
  using gin (to_tsvector('english', content));

-- B-tree for paper_id lookups
create index research_chunks_paper_id_idx on research_chunks (paper_id);
...
Run: `npx supabase db reset --dry-run` or check Supabase dashboard for `research_papers` and `research_chunks` tables.

- [ ] **Step 4: Commit**

```bash
git add supabase/migrations/007_research_knowledge_base.sql
git commit -m "feat: add research knowledge base schema with pgvector"
```

---

### Task 2: Python CLI Scaffolding

**Files:**
- Create: `tools/requirements.txt`
- Create: `tools/papers.py` (skeleton with argparse)
- Create: `tools/data/.gitkeep`
- Modify: `.gitignore`
...
- [ ] **Step 2: Create the CLI skeleton with argparse**

Create `tools/papers.py`:
...
def main():
    ...
    discover_cmd.add_argument("--count", type=int, default=80, help="Number of papers to output")
...
Create `tools/data/.gitkeep` (empty file).
...
- [ ] **Step 5: Verify CLI skeleton runs**

Run: `cd tools && python papers.py --help`
Expected: Shows usage with discover/ingest/stats subcommands.

- [ ] **Step 6: Commit**

```bash
git add tools/papers.py tools/requirements.txt tools/data/.gitkeep .gitignore
git commit -m "feat: scaffold research paper CLI tool"
```

---

### Task 3: Discovery Command — PubMed Search

**Files:**
- Modify: `tools/papers.py`

- [ ] **Step 1: Add PubMed search queries and esearch/efetch functions**

Add these constants and functions to `tools/papers.py` before `main()`:
...
- [ ] **Step 2: Add enrichment functions (OpenAlex + Unpaywall)**

Add after the PubMed functions:
...
def enrich_with_unpaywall(paper: dict) -> None:
    ...
    if not paper.get("doi"):
        return
    if paper.get("pmcid"):
        paper["full_text_url"] = f"https://www.ncbi.nlm.nih.gov/pmc/articles/{paper['pmcid']}/"
        paper["source"] = "pmc"
        return
    try:
        resp = requests.get(
...
- [ ] **Step 3: Implement the `discover()` function**

```python
def discover(count: int) -> None:
    """Search PubMed, enrich, rank, and write manifest."""
    print(f"Searching PubMed for reviews and meta-analyses (target: {count} papers)...")
    ...
    print(f"  Median citations: {papers[len(papers)//2]['citation_count'] if papers else 0}")
    print(f"  Top cited: {papers[0]['citation_count'] if papers else 0} — {papers[0]['title'][:80] if papers else 'N/A'}")
    ...
    print("Review the file and remove any unwanted papers, then run: python papers.py ingest")
```

- [ ] **Step 4: Test the discover command**

Run: `cd tools && source .venv/bin/activate && python papers.py discover --count 5`
Expected: Searches PubMed, fetches metadata, enriches, writes `tools/data/papers.json` with 5 papers.

- [ ] **Step 5: Commit**

```bash
git add tools/papers.py
git commit -m "feat: add paper discovery with PubMed, OpenAlex, Unpaywall"
```

---

### Task 4: Ingestion Command — Download & Parse

**Files:**
- Modify: `tools/papers.py`

- [ ] **Step 1: Add PMC XML download and section extraction**
...
- [ ] **Step 2: Add PDF download and Docling parsing**
...
- [ ] **Step 4: Add embedding and Supabase insertion**
...
def insert_paper(paper: dict, chunks: list[dict], embeddings: list[list[float]]) -> bool:
    ...
    if paper.get("doi"):
        existing = sb.table("research_papers").select("id").eq("doi", paper["doi"]).execute()
        if existing.data:
            print(f"  Skipping (already exists): {paper['title'][:60]}")
            return False

    # Determine source_type
    source_map = {"pmc": "full_text_pmc", "unpaywall": "full_text_pdf", "abstract_only": "abstract_only"}
...
- [ ] **Step 5: Implement the `ingest()` function**
...
def ingest() -> None:
    ...
    for paper in tqdm(papers, desc="Papers"):
        try:
            ...
            print(f"  Error processing {paper.get('title', 'unknown')[:60]}: {e}")
            errors += 1
...
```bash
git add tools/papers.py
git commit -m "feat: add paper ingestion with PMC/PDF parsing, chunking, embedding"
```
...
- [ ] **Step 1: Implement the `stats()` function**
...
def stats() -> None:
    ...
    papers = sb.table("research_papers").select("id, source_type, topics, citation_count").execute()
...
Run: `cd tools && source .venv/bin/activate && python papers.py stats`
Expected: Shows "Knowledge base is empty" if nothing ingested yet, or stats if data exists.

- [ ] **Step 3: Commit**

```bash
git add tools/papers.py
git commit -m "feat: add stats command to research paper CLI"
```

---

### Task 6: Install @ai-sdk/openai
...
Run: `grep ai-sdk/openai package.json`
Expected: Shows `"@ai-sdk/openai": "^x.x.x"` in dependencies.

- [ ] **Step 3: Commit**

```bash
git add package.json package-lock.json
git commit -m "feat: add @ai-sdk/openai for embedding support"
```

---

### Task 7: Search Research Coach Tool

**Files:**
- Create: `src/lib/chat/tools/search-research.ts`
- Create: `__tests__/lib/chat/tools/search-research.test.ts`
      ...
      data: [
        {
          id: "chunk-1",
          paper_id: "paper-1",
          paper_title: "Dose-response of RT volume",
          authors: "Schoenfeld BJ, Ogborn D",
          year: 2017,
          journal: "J Strength Cond Res",
          doi: "10.1519/JSC.001",
          content: "10+ sets per week produced greater hypertrophy",
          section: "results",
          similarity: 0.89,
        },
      ],
...
describe("search-research tool", () => {
  it("returns a tool with correct description and schema", () => {
    const t = getSearchResearchTool();
    expect(t.description).toContain("research");
    expect(t.inputSchema).toBeDefined();
  });

  it("executes and returns formatted results", async () => {
    const t = getSearchResearchTool();
    const results = await t.execute({ query: "hypertrophy volume", max_results: 5 });
    expect(results).toHaveLength(1);
    expect(results[0]).toMatchObject({
      title: "Dose-response of RT volume",
      authors: "Schoenfeld BJ, Ogborn D",
      year: 2017,
      similarity: 0.89,
    });
...
Run: `npm test -- --run __tests__/lib/chat/tools/search-research.test.ts`
...
export function getSearchResearchTool() {
  return tool({
    description:
      "Search exercise science research papers for evidence-based recommendations. " +
      "Use when making training, nutrition, or recovery recommendations to back them up with citations. " +
      "Returns relevant paper excerpts with authors and year for citation.",
    inputSchema: z.object({
      query: z.string().describe(
        "Natural language search query, e.g. 'optimal sets per week for hypertrophy' " +
        "or 'creatine supplementation effects on strength'"
      ),
      max_results: z
        .number()
        .optional()
    ...
    execute: async ({ query, max_results }) => {
      const count = Math.min(Math.max(max_results, 1), 10);
...
```bash
git add src/lib/chat/tools/search-research.ts __tests__/lib/chat/tools/search-research.test.ts
git commit -m "feat: add search_research tool for AI coach"
```

---

### Task 8: Wire Tool into Chat Route
...
- [ ] **Step 1: Export from tools/index.ts**
...
- [ ] **Step 2: Wire into chat route**

In `src/app/api/chat/route.ts`, add to the import block:
...
And add to the tools object inside `streamText()`:
...
Note: `getSearchResearchTool()` takes no `userId` — research papers are global.

- [ ] **Step 3: Update system prompt with citation guidelines**

In `src/lib/chat/system-prompt.ts`, add these lines at the end of the guidelines section (before `return lines.join("\n")`):
...
```bash
git add src/lib/chat/tools/index.ts src/app/api/chat/route.ts src/lib/chat/system-prompt.ts
git commit -m "feat: wire search_research tool into AI coach"
```
...
Run: `npm test`
Expected: All tests pass, including the new search-research tool test.

- [ ] **Step 2: Run discover with small count to test pipeline**

Run: `cd tools && source .venv/bin/activate && python papers.py discover --count 5`
Expected: Writes `tools/data/papers.json` with 5 papers, shows citation counts and OA availability.

- [ ] **Step 3: Run ingest on the 5-paper test set**

Run: `cd tools && python papers.py ingest`
Expected: Inserts papers and chunks into Supabase, shows progress and final counts.

- [ ] **Step 4: Run stats to verify**

Run: `cd tools && python papers.py stats`
Expected: Shows paper count, chunk count, source type breakdown, topic breakdown.

- [ ] **Step 5: Final commit**

```bash
git add -A
git commit -m "feat: research paper pipeline — discover, ingest, and coach search tool"
```
