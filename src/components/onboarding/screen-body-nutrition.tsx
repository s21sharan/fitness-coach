...
export const SCREEN_BODY_NUTRITION_SUBTITLE =
  "Body composition goal lives separately from performance — we'll flag conflicts when they fight each other.";
...
export function ScreenBodyNutrition({ profile, onUpdate }: ScreenBodyNutritionProps) {
  ...
  return (
    <div style={{ maxWidth: 660, margin: "0 auto", width: "100%", display: "flex", flexDirection: "column", gap: 22 }}>
      ...
      <div>
        <p style={{ ...labelStyle, marginBottom: 10 }}>Protein target</p>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
          {PROTEIN_TIERS.map((opt) => {
            const selected = bn.protein_tier === opt.value;
            return (
              <button
                key={opt.value}
                type="button"
                onClick={() => set({ protein_tier: opt.value })}
                style={{
                  padding: "12px 14px",
                  borderRadius: "var(--r-md)",
                  border: selected ? "2px solid var(--ink)" : "1.5px solid var(--line)",
      ...
      <div>
        <p style={{ ...labelStyle, marginBottom: 10 }}>Diet style</p>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          {DIET_STYLES.map((opt) => {
            const chosen = bn.diet_style === opt.value;
            return (
              <button
                key={opt.value}
                type="button"
                onClick={() => set({ diet_style: opt.value })}
                style={{
                  padding: "8px 14px",
                  borderRadius: 999,
